...
struct AIReportView: View {
    ...
    private func makeRefactoringPrompt() -> String {
        ...
        ## 리팩토링 요청사항
        1. 순환 의존성부터 끊어주세요. 프로토콜 도입 또는 의존 방향 역전으로 해결해주세요.
        2. 위에 나열된 메모리 누수 패턴을 [weak self] / weak delegate / Timer 무효화로 수정해주세요.
        3. 레이어 위반 파일을 역할에 맞는 레이어로 분리해주세요.
        4. 나머지 할 일 항목을 우선순위대로 처리해주세요.
        5. 각 수정마다 변경 이유와 예상 효과를 간단히 설명해주세요.
        6. 수정 후 건강 점수가 어느 정도 오를지 추정해주세요.
        """
    }
